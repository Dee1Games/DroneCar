...
namespace Plugins.RaycastPro.Demo.Scripts
{
    ...
    public class SteeringController : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (movable)
            {
                if (Vector3.Distance(transform.position, detector.destination.position) > arriveDistance)
                {
                    rb.velocity = Vector3.Lerp(rb.velocity, detector.SteeringDirection * speed,
                        1 - Mathf.Exp(-accelerationSpeed * Time.deltaTime));
                }
                else
                {
                    rb.velocity = Vector3.Lerp(rb.velocity, Vector3.zero,
                        1 - Mathf.Exp(-accelerationSpeed * Time.deltaTime));
                }
            }
